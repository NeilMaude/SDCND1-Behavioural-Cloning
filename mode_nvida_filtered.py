...
from utils import gen_batches, train_validation_split
...
flags.DEFINE_integer('centre_count', 7, 'The number of centre-lane driving files.')
flags.DEFINE_integer('left_recover_count', 3, 'The number of left-of-centre recovery files.')
flags.DEFINE_integer('right_recover_count', 3, 'The number of right-of-centre recovery files.')
...
def main(_):
    ...
    for file_num in range(1,FLAGS.left_recover_count+1):
        ...
        for i in range(len(data)):
            if data[i][0] != 'center':  # ignore the header row, if there is one
                if (float(data[i][3]) > 0.0) and (float(data[i][3]) < 0.55) :      # only interested in +ve steering angle frames, within a certain range
                    # sort out the image path, if using the Udacity provided data
                    s = str(data[i][0])             # get the filename from the csv data
                    s = s[s.rfind('/')+1:]          # trim off any paths
                    s = FLAGS.imgs_dir + s          # put back the path to the images location
    ...
    for file_num in range(1, FLAGS.right_recover_count + 1):
        ...
        for i in range(len(data)):
            if data[i][0] != 'center':  # ignore the header row, if there is one
                if (float(data[i][3]) < 0.0) and (float(data[i][3]) > -0.55):  # only interested in -ve steering angle frames, with a range
    ...
    history = model.fit_generator(gen_batches(X_train, y_train, FLAGS.batch_size),
                                  n_samples_per_epoch,
                                  FLAGS.num_epochs,
                                  validation_data=gen_batches(X_valid, y_valid, FLAGS.batch_size),
    ...
    json = model.to_json()
    model.save_weights('save_nvidia_filtered/model.h5')
    with open('save_nvidia_filtered/model.json', 'w') as f:
        f.write(json)
